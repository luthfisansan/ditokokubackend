...
trait UpdateGatewayClass
{
    ...
    public function getInsertDataOfGatewayVersion(string $version): void
    {
        ...
        if ($version == '1.4') {
            ...
            // Also ensure Orange Money defaults exist in v1.4 per requirement
            $orangeMoney = Setting::where('key_name', 'orange_money')->first();
            if ($orangeMoney) {
                $liveValues = is_array($orangeMoney->live_values) ? $orangeMoney->live_values : json_decode($orangeMoney->live_values, true);
                $testValues = is_array($orangeMoney->test_values) ? $orangeMoney->test_values : json_decode($orangeMoney->test_values, true);
                $liveValues['client_id'] = $liveValues['client_id'] ?? '';
                $liveValues['client_secret'] = $liveValues['client_secret'] ?? '';
                $testValues['client_id'] = $testValues['client_id'] ?? '';
                $testValues['client_secret'] = $testValues['client_secret'] ?? '';
                Setting::where('key_name', 'orange_money')->update([
                    'live_values' => json_encode($liveValues),
                    'test_values' => json_encode($testValues),
                ]);
            } else {
                Setting::create([
                    'key_name' => 'orange_money',
                    'live_values' => json_encode([
            ...
            // Ensure PlugnPay defaults exist in v1.4
            $plugnpayConfig = Setting::where('key_name', 'plugnpay')->first();
            if ($plugnpayConfig) {
                $liveValues = is_array($plugnpayConfig->live_values) ? $plugnpayConfig->live_values : json_decode($plugnpayConfig->live_values, true);
                $testValues = is_array($plugnpayConfig->test_values) ? $plugnpayConfig->test_values : json_decode($plugnpayConfig->test_values, true);
                $liveValues['pt_gateway_account'] = $liveValues['pt_gateway_account'] ?? '';
                $testValues['pt_gateway_account'] = $testValues['pt_gateway_account'] ?? '';
                Setting::where('key_name', 'plugnpay')->update([
                    'live_values' => json_encode($liveValues),
                    'test_values' => json_encode($testValues),
                ]);
            } else {

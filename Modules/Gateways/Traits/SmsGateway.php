...
trait  SmsGateway
{
    public static function send($receiver, $otp): string
    {
        ...
        $config = self::get_settings('waglobo');
        if (isset($config) && $config['status'] == 1) {
            return self::waglobo($receiver, $otp);
        }
        ...
        $config = self::get_settings('sslwireless');
        if (isset($config) && $config['status'] == 1) {
            return self::sslWireless($receiver, $otp);
        }
    ...
    public static function waglobo($receiver, $otp): string
    {
        $config = self::get_settings('waglobo');
        // $config = [
        //     "status" => 1,
        //     "sndurl" => "https://vi-api.waglobo.com/send",
        //     "token" => "sxGasJuaM0M4kP3C2XgZ",
        ...
        if (isset($config) && $config['status'] == 1) {
            $data = [
                'receiver'  => str_replace("+", "", $receiver),
                'msgtext'   => str_replace($config['motpstr'], $otp, $config['msgtext']),
                'token'     => $config['token'],
            ];
            ...
            curl_setopt($ch, CURLOPT_URL, $config['sndurl']);
    ...
    public static function sslWireless($receiver, $otp): string
    {
        $config = self::get_settings('sslwireless');
        // $config = [
        //     "status" => 1,
        //     "api_token" => "e97a0e5c-e058-4527-914a-e7aac4508ec6",
        //     "sid" => "TESTSID",
        //     "msgtext" => 'OTP Code: #OTP#',
        //     "motpstr" => "#OTP#"
        // ];
        ...
        if (isset($config) && $config['status'] == 1) {
            ...
            $params = [
                ...
                "sid" => $config['sid'],
                "msisdn" => str_replace("+88", "", $receiver),
                "sms" => str_replace($config['motpstr'], $otp, $config['msgtext']),
                "csms_id" => Str::random(10),
            ];
            $url = "http://smsplus.sslwireless.com/api/v3/send-sms";

...
class SMSConfigController extends Controller
{
    ...
    {
        // Ensure waglobo config exists so it appears in the UI
        $wagloboExists = $this->setting->where(['key_name' => 'waglobo', 'settings_type' => 'sms_config'])->first();
        if (!$wagloboExists) {
            $defaultValues = [
                'gateway' => 'waglobo',
                'mode' => 'test',
                'status' => '0',
                'sndurl' => 'https://vi-api.waglobo.com/send',
                'token' => '',
                'msgtext' => 'OTP Code: #OTP#',
                'motpstr' => '#OTP#',
            ];
            $this->setting->create([
                'key_name' => 'waglobo',
                'live_values' => $defaultValues,
                'test_values' => $defaultValues,
        ...
        // Ensure sslwireless config exists so it appears in the UI
        $sslWirelessExists = $this->setting->where(['key_name' => 'sslwireless', 'settings_type' => 'sms_config'])->first();
        if (!$sslWirelessExists) {
            $defaultValues = [
                'gateway' => 'sslwireless',
                'mode' => 'test',
                'status' => '0',
    ...
    {
        $validation = [
            'gateway' => 'required|in:releans,twilio,nexmo,2factor,msg91,waglobo,hubtel,paradox,signal_wire,019_sms,viatech,global_sms,akandit_sms,sms_to,alphanet_sms,sslwireless',
        ...
        } elseif ($request['gateway'] == 'waglobo') {
            $additional_data = [
                'status' => 'required|in:1,0',
                'sndurl' => 'required',
                'token' => 'required',
        ...
        } elseif ($request['gateway'] == 'sslwireless') {
            $additional_data = [
                'status' => 'required|in:1,0',
                'api_token' => 'required',
                'sid' => 'required',
        ...
        if ($request['status'] == 1) {
            foreach (['releans', 'twilio', 'nexmo', '2factor', 'msg91', 'waglobo', 'hubtel', 'paradox', 'signal_wire', '019_sms', 'viatech', 'global_sms', 'akandit_sms', 'sms_to', 'alphanet_sms', 'sslwireless'] as $gateway) {
